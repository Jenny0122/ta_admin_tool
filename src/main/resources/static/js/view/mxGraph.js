...
/* 언어 분석 */
const MORPHOLOGICAL_ANALYSIS = "MORPHOLOGICAL_ANALYSIS";
const BLACKWORD_REMOVAL = "BLACKWORD_REMOVAL";
const WHITEWORD_EXTRACTION = "WHITEWORD_EXTRACTION";
const ENTITY_EXTRACTION = "ENTITY_EXTRACTION";
const LINGUISTIC_ANALYSIS = [PREPROCESSING, MORPHOLOGICAL_ANALYSIS, BLACKWORD_REMOVAL, WHITEWORD_EXTRACTION, ENTITY_EXTRACTION];

/* 자질 추출 */
const KEYWORD_EXTRACTION = "KEYWORD_EXTRACTION";
const STOPWORD_RECOMMENDATION = "STOPWORD_RECOMMENDATION";

/* 응용 분석 */
const DOCUMENT_CLUSTERING = "DOCUMENT_CLUSTERING";
const WORD_CLUSTERING = "WORD_CLUSTERING";
const RELATED_KEYWORD_EXTRACTION = "RELATED_KEYWORD_EXTRACTION";
const EXTRACTIVE_SUMMARIZATION = "EXTRACTIVE_SUMMARIZATION";

/* label이 필요한 응용 분석 */
const DOCUMENT_CLASSIFICATION = "DOCUMENT_CLASSIFICATION";
const ABSTRACTIVE_SUMMARIZATION = "ABSTRACTIVE_SUMMARIZATION";
const TOPIC_ANALYSIS = "TOPIC_ANALYSIS";
const APPLICATION_ANALYSIS = [DOCUMENT_CLASSIFICATION, WORD_CLUSTERING, DOCUMENT_CLUSTERING, RELATED_KEYWORD_EXTRACTION];
const NEED_LABEL = [DOCUMENT_CLASSIFICATION];
...
mxGraph.prototype.getEdgeValidationError = function(edge, source, target)
{
	...
	if (source != null && target != null)
	{
		...
		if (!this.multigraph)
		{
		    const sourceType = convertType_IdToType(source.getId());
            const targetType = convertType_IdToType(target.getId());
            ...
            // source == 언어 분석(array)
            // target == 추출 요약, 생성 요약, 토픽 분석
            // return: error message
            else if (LINGUISTIC_ANALYSIS.includes(sourceType))
            {
                if
                (
                    targetType == EXTRACTIVE_SUMMARIZATION ||
                    targetType == ABSTRACTIVE_SUMMARIZATION ||
                    targetType == TOPIC_ANALYSIS
                )
                {
                    error += (mxResources.get(this.alreadyConnectedResource) ||
                        this.alreadyConnectedResource) + '\n';
            ...
            // target == 모든 태스크
            // return: error message
            else if (sourceType == STOPWORD_RECOMMENDATION)
            {
                error += (mxResources.get(this.alreadyConnectedResource) ||
                    this.alreadyConnectedResource) + '\n';
            }
            // source == 키워드
            // target == 언어 분석(array), 불용어 추천, 추출 요약, 생성 요약, 토픽 분석
            // return: error message
            else if (sourceType == KEYWORD_EXTRACTION)
            {
                if
                (
                    LINGUISTIC_ANALYSIS.includes(targetType) ||
                    targetType == STOPWORD_RECOMMENDATION ||
                    targetType == EXTRACTIVE_SUMMARIZATION ||
                    targetType == ABSTRACTIVE_SUMMARIZATION ||
                    targetType == TOPIC_ANALYSIS
                )
            ...
            // target == 모든 태스크
            // return: error message
            else if (APPLICATION_ANALYSIS.includes(sourceType))
            {
                error += (mxResources.get(this.alreadyConnectedResource) ||
                    this.alreadyConnectedResource) + '\n';
            }
            ...
            // target == 모든 태스크
            // return: error message
            else if (sourceType == EXTRACTIVE_SUMMARIZATION)
            {
                error += (mxResources.get(this.alreadyConnectedResource) ||
                    this.alreadyConnectedResource) + '\n';
            }
            ...
            // target == 모든 태스크
            // return: error message
            else if (sourceType == ABSTRACTIVE_SUMMARIZATION)
            {
                error += (mxResources.get(this.alreadyConnectedResource) ||
                    this.alreadyConnectedResource) + '\n';
            }
            ...
            // target == 모든 태스크
            // return: error message
            else if (sourceType == TOPIC_ANALYSIS)
            {
                error += (mxResources.get(this.alreadyConnectedResource) ||
                    this.alreadyConnectedResource) + '\n';
            }
            // source == 모든 태스크
            // target == COLLECTION, BLANK
            // return: error message
            if (targetType == COLLECTION || targetType == BLANK_VAL)
            {
                error += (mxResources.get(this.alreadyConnectedResource) ||
                    this.alreadyConnectedResource) + '\n';
            }
            // source == 컬렉션이 연결되지 않은 태스크
            // target == 문서 분류
            // return: error message
            if (targetType == DOCUMENT_CLASSIFICATION)
            {
                const isCollection = _findCollectionCell(source);
                if(!isCollection) {
                    error += ("컬렉션이 연결되어 있지 않습니다.") + '\n';
                }
            }
		}
